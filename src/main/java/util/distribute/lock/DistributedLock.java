...
public class DistributedLock {
     ...
     * @throws InterruptedException 线程中断异常
     */
    public synchronized boolean acquire(Jedis jedisCmd) throws InterruptedException {
        ...
        while (timeout >= 0) {
             ...
             *      否则，Redis中已存在锁，获取锁失败;
             */
            if (jedisCmd.setnx(lockKey, tLock.toString()) == 1) {
                this.lock = tLock;
                return true;
            }
             ...
             *      否则，通过自旋获取锁
             */
            final String currentLockValue = jedisCmd.get(lockKey);                                                      //获取Redis中已存在的锁的值
            ...
            if (currentLock.isExpiredOrMine(lockUUID)) {
                String originLockValue = jedisCmd.getSet(lockKey, tLock.toString());                                    //在锁确定获取的情况下设置redis
                /**
                 * 这里还有个前置条件:
                 *      会对原始锁进行校验，jedis.get()和jedis.getSet()获取的锁必须是同一锁，重新获取锁才成功，防止get到getSet这段时间其他线程获取锁
                 */
            ...
            timeout = timeout - default_acquire_resolution_millis;
    ...
    public synchronized void release(Jedis jedisCmd) {
        if (isLocked()) {
            //存在一种情况，当前线程阻塞很长时间后再次执行，此时该线程持有的锁已经超时，并且其它线程获取了锁。这时当前线程就不应该再删除该锁
            if (this.lock.isExpired()) {//当前线程持有的锁已经超时
                final Lock lock = Lock.fromString(jedisCmd.get(lockKey));//获取Redis中已存在的锁
